...
from module import DeepEXON, FocalLoss, initial_model
...
def train_cv():
    seed, cell_line, seq_type, emb_type_1, emb_type_2, max_len, epoch_num, batch_size, patience, threshold = \
         args.seed, args.cell_line, args.seq_type, args.emb_type_1, args.emb_type_2, args.max_len, args.epoch_num, args.batch_size, args.patience, args.threshold
...
#         os.makedirs(save_path)
    ...
    # Load datasets
    train_dataset, test_dataset = load_dataset(seq_type, emb_type_1, cell_line, max_len, seed)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
                             worker_init_fn=np.random.seed(seed))
    # Model
    model = DeepEXON(args, max_len, train_dataset.emb_dim, kernel_size, head_num, hidden_dim, layer_num, attn_drop,
                    lstm_drop,
                    linear_drop)
...
def cal_by_epoch(mode, model, loader, loss, optimizer=None, threshold=0.5):
    # Model on train mode
    model.train() if mode == 'train' else model.eval()
    all_trues, all_scores= [],[]
    losses, sample_num = 0.0, 0
    for iter_idx, (X, X_exon, X_EXON_one_hot, y) in enumerate(loader):
        sample_num += y.size(0)
        ...
        with torch.no_grad():
            ...
            X_exon = Variable(X_exon.to(device))
            X_EXON_one_hot = Variable(X_EXON_one_hot.to(device))
            y_var = torch.autograd.Variable(y.to(device).float())

        # compute output
        model = model.to(device)
        output = model(X_var, X_exon, X_EXON_one_hot).view(-1)
